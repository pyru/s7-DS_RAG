...
def read_file(path: str) -> dict:
    """Read a file from the sandbox. Supports plain text, PDF, and DOCX — text is extracted automatically. Example: read_file("notes.txt"), read_file("RAG/budget_speech.docx")."""
    p = _safe(path)
    text = _extract_text_from_path(p)
    suffix = p.suffix.lower()
    encoding = "extracted" if suffix in (".pdf", ".docx") else "utf-8"
    return {
        "path": path,
...
def _extract_text_from_path(p: Path) -> str:
    """Extract plain text from .pdf, .docx, or any UTF-8 text file."""
    suffix = p.suffix.lower()
    if suffix == ".pdf":
        try:
            from pypdf import PdfReader
        except ImportError:
            raise ImportError("pypdf required for PDF support: uv add pypdf")
    ...
    if suffix == ".docx":
        try:
            ...
            raise ImportError("python-docx required for DOCX support: uv add python-docx")
        doc = Document(str(p))
...
def _read_for_index(path: str) -> tuple[str, str]:
    ...
    p = _safe(path)
    return _extract_text_from_path(p), f"sandbox:{path}"
