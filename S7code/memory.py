...
def add_fact(
    ...
    *,
    ...
    Pass a pre-computed `embedding` to skip the gateway call (used by the
    bulk-indexing script which batch-embeds chunks for throughput)."""
...
def add_facts_bulk(
    ...
    """Write multiple fact items in a single load/save cycle.

    Each dict must have keys: descriptor, source, run_id.
    Optional keys: value, keywords, goal_id, embedding.

    Returns the number of items written. Used by the corpus indexer to avoid
    a per-chunk load+save cycle which becomes unbearably slow on large corpora
    (a 1,561-chunk file would otherwise re-read a 100 MB memory.json 1,561×).
    """
    new_items: list[MemoryItem] = []
    for d in items_data:
        embedding = d.get("embedding")
        kw = d.get("keywords") or list(_tokens(d["descriptor"]))[:10]
        item = MemoryItem(
            id=new_id("mem"),
            kind="fact",
            keywords=list({k.lower() for k in kw}),
